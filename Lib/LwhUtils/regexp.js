...
// 邮箱
export const validateEmail = str => /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/.test(str)
...
// 固定电话
export const validateFixedphone = str => /^(\(\d{3,4}\)|\d{3,4}-|\s)?\d{7,14}$/.test(str)

// 身份证号（18位）
export const validateIdCard = str => /^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$/.test(str)

// URL
export const validateUrl = str => /^(https?|ftp|file):\/\/[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]$/.test(str)

// IPv4地址
export const validateIPv4 = str => /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(str)
...
export const validatePwd = str => /(?!^\d+$)(?!^[A-Za-z]+$)(?!^[^A-Za-z0-9]+$)(?!^.*[\u4E00-\u9FA5].*$)^\S{6,20}$/.test(str)
...
// 只能包含中文、字母，数字
export const validateCNCharAlphaNum = str => /^[\u4e00-\u9fa5a-zA-Z0-9]*$/i.test(str)
